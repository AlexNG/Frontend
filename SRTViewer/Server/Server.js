...
class Server {
  ...
  setupStaticFeature() {
    ...
    if (Array.isArray(contentBase)) {
      ...
      console.log('serve static 1');

      if (this.options.serveIndexes) {
        this.options.serveIndexes.forEach(arr => {
            const path = arr[1].startsWith('!') ? arr[1].substr(1) : contentBase + arr[1];
            this.app.use(arr[0], serveIndex(path));
            this.app.use(arr[0], express.static(path));
          }
        );
      }
      ...
      // devServer.app.use('/SRT', serveIndex(paths.appPublic + '\\SRT'));      
    }
  }
